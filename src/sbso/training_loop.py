...
class SBSOTrainer:
    ...
    def run_stage3(self, play_match: Callable[["SBSOTrainer", int, Any], tuple]) -> dict:
        """Stage 3: one 'episode' = one full match (many MCTS decisions), matching the
        confirmed scope ("episodes" means full matches). Reuses the exact same
        scheduler/checkpoint/win-history bookkeeping as run() - only how one episode's
        training_pairs/outcome get produced changes. play_match(trainer, episode,
        opponent) -> (won, training_pairs) is provided by a MatchRunner
        (src.sbso.match_runner) - this method has no PyBullet-specific knowledge,
        matching Stage 1/2's separation of concerns.

        DSPy recompile-eligibility and checkpoint snapshotting are checked ONCE PER
        MATCH here (episode-level) - see match_runner.py's docstring for the reasoning
        and the seam to revisit if mid-match recompiles are wanted later."""
        for ep in range(self.episodes):
            opp = self.opponent_pool.sample(ep, self.checkpoint_mgr.has_checkpoint())
            won, pairs = play_match(self, ep, opp)

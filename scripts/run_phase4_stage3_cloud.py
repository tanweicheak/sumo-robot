...
def _add_stage3_cloud_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--judge-server-url", required=True,
                         help="SGLang server for the JUDGE model (Llama-3.1-8B-Instruct per "
                              "inference.yaml), e.g. http://127.0.0.1:30001 - this is the only "
                              "server Stage 3 training actually needs.")
    parser.add_argument("--agent-server-url", default=None,
                         help="SGLang server for the AGENT model (Phi-4-mini per inference.yaml), "
                              "e.g. http://127.0.0.1:30000 - ONLY needed if --real-dspy is set, "
                              "since DSPy compiles the Strategy Agent's prompt, a different model "
                              "from the Judge. Do not point this at the judge server by mistake.")
    parser.add_argument("--sim-budget", type=int, required=True,
    ...
    parser.add_argument("--horizon", type=int, required=True, help="Same reasoning as --sim-budget.")
    parser.add_argument("--cycles-per-node", type=int, required=True, help="Same reasoning as --sim-budget.")
    parser.add_argument("--real-dspy", action="store_true", help="Use RealDSPyCompiler against SGLang instead of MockDSPyCompiler")
    parser.add_argument("--k-episodes", type=int, default=None, help="Override if config's dspy_recompilation.k_rollout_batches is null")
    parser.add_argument("--window-w", type=int, default=None, help="Override if config's rolling_window_w is null")
    parser.add_argument("--delta", type=float, default=None, help="Override if config's reward_drop_threshold_delta is null")
...
def main() -> None:
    config, ctx, args = build_run(
        phase="phase4_stage3_cloud",
        description=__doc__,
        extra_args=_add_stage3_cloud_args,
    )
    print(f"[stage3-cloud] run_id={ctx.run_id}")
    ...
    judge = SGLangJudge(server_url=args.judge_server_url)
    ...
    if args.real_dspy:
        if not args.agent_server_url:
            raise SystemExit("--real-dspy requires --agent-server-url (the Phi-4-mini SGLang "
                              "server, NOT the judge server)")
        dspy_compiler = RealDSPyCompiler(sglang_api_base=f"{args.agent_server_url}/v1")
    else:

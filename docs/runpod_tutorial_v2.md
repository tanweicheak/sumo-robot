...
7. On the pod (check `python3 --version` first — if it's not 3.12.x, use the
   same venv setup as §7 below before installing anything, or `pybullet` will
   try to build from source and may fail):
   ```bash
   git clone <your-repo-url> sumo-sbso
   cd sumo-sbso
   python3.12 -m venv /workspace/venv && source /workspace/venv/bin/activate   # skip if python3 is already 3.12.x

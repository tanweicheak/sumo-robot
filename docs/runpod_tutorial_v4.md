...
## 9. (Optional) Log into wandb — currently unused by any script, see note

```bash
wandb login
```
Paste your API key from wandb.ai/authorize if you want it available for
future use. **Correction:** an earlier version of this tutorial said every
script treats `--use-wandb` as optional — that flag doesn't exist.
`run_phase4_pilot.py`'s argparse only accepts `--config`, `--results-dir`,
`--episodes-override`, `--checkpoint-interval-override`. A full grep of
every script plus a search across all of `graph.json`'s indexed nodes
turned up no `wandb.init()`/`wandb.log()` call anywhere in the codebase —
the only other mention of wandb at all is an optional-login line in an
older doc (`docs/runpod_tutorial_v1.md`). `wandb` is installed (it's in
`requirements_runpod.txt`) and explicitly disabled in `lora_finetune.py`'s
`TrainingArguments(report_to=[])`, but nothing currently reports to it.
Safe to skip this section entirely for now.
...
```bash
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_pilot.yaml --episodes-override 100
```

(`--episodes-override 100` keeps this short and cheap — a genuine
calibration run, not the full 500. No `--use-wandb` flag — see the
correction in §9, it isn't a real option.)
...
supports `phase4_pilot.yaml`'s reduced opponent scope — pointing it at
`phase4_full_sbso.yaml` or any ablation config raised
`KeyError: 'opponent_pool'/'pilot_scope'` immediately. This has since been
patched (small change: the opponent factory no longer reads a pilot-only
config key, and `OpponentPool` now receives the real `SelfCheckpointManager`
+ `full_run_targets` so self-checkpoint opponents actually get sampled for
the full run). Confirm that patch is applied to this pod's checkout — diff
your `scripts/run_phase4_pilot.py` against the corrected version in your
project notes — before running any of the 5 commands below. See also the
noted limitation there: self-checkpoint opponents are correctly used during
MCTS search after the patch, but still fall back to a rule-based policy for
the real match continuation — a documented, pre-existing gap, not something
this patch introduces or fixes.

One command per variant, same script, different config each time:

```bash
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_full_sbso.yaml
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_ablation_no_sa.yaml
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_ablation_no_mcts.yaml
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_ablation_no_dspy.yaml
python -m scripts.run_phase4_pilot --config <verified-path>/phase4_ablation_no_judge.yaml
```
...
(printed at the end of §13) told you roughly what to expect for each. (No
`wandb` cross-check here — see §9's correction, it isn't tracking anything.)
